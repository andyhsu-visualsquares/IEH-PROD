const baseUrl = 'https://5d18-210-6-231-122.ngrok-free.app/erunapi_artisan_test/api.asmx'
function Schedule_POS_to_NS_AIL() {
    try {
        ...
        nlapiLogExecution(
            'Error',
            'Schedule_POS_Member_GetChanges_J',
            Err.toJSON ? Err : Err.stack ? Err.stack : Err.toString()
        )
    }
    try {
...
function Schedule_POS_Member_GetChanges_J() {
    nlapiLogExecution('audit', 'Start Memeber get')
    ...
    //Product_AddNew_J
    // var url = "http://ieh.softether.net:8888/erunapi_artisan/api.asmx/Member_GetChanges_J";
    // var url = "http://103.225.9.37:8888/erunapi_artisan/api.asmx/Member_GetChanges_J";
    // var url = "https://eruntestapi.free.beeceptor.com/api.asmx/Member_GetChanges_J";
    // https://eruntestapi.free.beeceptor.com
    //! 26-Sep-2023
    // var url = 'https://ithpos.app/erunapi_artisan/api.asmx/Member_GetChanges_J'
    var url = baseUrl + '/Member_GetChanges_J'
    ...
    var header = {
        ...
        cookie: LoginRes,
        token: getSessionId(LoginRes),
    }

    // var requestBody = {};

    var lastSyncDateTime = nlapiLookupField('customrecordintegrationlib', 98, 'custrecordintegrationlibdate')
    ...
    var vBeginTime = starttime
    var vEndTime = endtime

    // vBeginTime = '2022/06/14 23:59:59';
    // vEndTime = '2022/12/20 23:59:59';

    var requestBody = 'vBeginTime=' + vBeginTime + '&vEndTime=' + vEndTime
    ...
    nlapiLogExecution('debug', 'vBeginTime: ' + vBeginTime, 'vEndTime: ' + vEndTime)
    nlapiLogExecution(
        'debug',
        'POST: ',
        'header: ' + JSON.stringify(header) + '----- requestBody: ' + JSON.stringify(requestBody)
    )
    // var response = nlapiRequestURL(url,JSON.stringify(requestBody),header,'POST');
    var response = nlapiRequestURL(url, requestBody, header, 'POST')
    // var response = nlapiRequestURLWithCredentials(null,url,JSON.stringify(requestBody),header,'POST');
    ...
    if (ResponseBody != '{"d":""}') {
        ...
        if (memberList) {
            for (var i = 0; i < memberList.length; i++) {
                ...
                var search = nlapiLoadSearch('customer', 'customsearchcustomercreatefrompos_2')
                checkGovernance()
                ...
                if (searchresults.length > 0) {
                    ...
                    recCus.setFieldValue('cseg1', 86)

                    recCus.setFieldValue('category', 2)
                    recCus.setFieldValue('custentity_customer_sub_category', SubCat)
                    recCus.setFieldValue('subsidiary', 14)
                ...
                nlapiLogExecution('debug', 'SGR: ', SGR)
                ...
                filters[1] = new nlobjSearchFilter('custrecordcustomersubcat_ail', null, 'is', 'T')
                ...
                for (var m = 0; SubCatSearchresults.length > 0 && SubCatSearchresults && m == 0; m++) {
                    ...
                    var SubCatName = SubCatSearchresults[m].getValue(ResultColumns[1])
                    nlapiLogExecution('debug', 'SubCat: ' + SubCat, 'SubCatName: ' + SubCatName)
                }
                recCus.setFieldValue('custentity_customer_sub_category', SubCat)
        ...
        nlapiSubmitField('customrecordintegrationlib', 98, 'custrecordintegrationlibdate', now)
    }
    nlapiLogExecution('audit', 'End Memeber get')
}
...
function Schedule_POS_Xact_GetSalesChanges_J() {
    ...
    // var url = "http://ieh.softether.net:8888/erunapi_artisan/api.asmx/Xact_GetSalesHistoryV2_J";
    //! 26-Sep-2023
    // var url = 'https://ithpos.app/erunapi_artisan/api.asmx/Xact_GetSalesHistoryV2_J'
    var url = baseUrl + '/Xact_GetSalesHistoryV2_J'
    ...
    var lastSyncDateTime = nlapiLookupField('customrecordintegrationlib', 99, 'custrecordintegrationlibdate')
    var lastSyncDateTimeDateEnd1 = nlapiLookupField('customrecordintegrationlib', 100, 'custrecordintegrationlibdate')
    ...
    for (
        var xx = 4;
        xx >= 0;
        xx-- // for (var xx = 40; xx > 0; xx--)
        // for (var xx = 97; xx > 95; xx--) // 92 = 07/11/2021
    ) {
        // GetDateKeyyyyymmddLastXDay(xx)
        var lastSyncDateTime = GetDateKeyyyyymmddLastXDay(xx) + ' 00:00:00'
        var lastSyncDateTimeDateEnd = GetDateKeyyyyymmddLastXDay(xx) + ' 23:59:00'
        // ?vBeginTime=2019/03/29 11:30:00&vEndTime=2019/03/29 23:59:59

        // lastSyncDateTime = '2022/11/20 00:00:00';
        // lastSyncDateTimeDateEnd = '2022/05/30 23:59:59';
        // vBeginTime:
        // 2020/01/17 13:40:00
        // vEndTime:
        // 2020/01/17 14:00:00

        nlapiLogExecution('debug', 'url2', url)
        var today = new Date()
        // var now = GetDateKey(today);
        var endtime = GetDateKeyDateEnd(lastSyncDateTime)
        var starttime = GetDateKeyDateStart(lastSyncDateTime)
        // nlapiLogExecution("debug","now",now);
        var requestBody = 'vBeginTime=' + starttime + '&vEndTime=' + lastSyncDateTimeDateEnd + '&bLU=true&bDVD=true'
        ...
        if (SalesList && SalesList != null && SalesList != 'null' && SalesList != '') {
            ...
            for (var i = 0; i < SalesList.length; i++) {
                ...
                if (ShopNum != 99 && ShopNum != 0 && ShopNum != 900) {
                    ...
                    // if (ShopNum != '900')
                    {
                        ...
                        */
                        ...
                        checkGovernance()
                        var Locationsearchfilter = LocationSearch.getFilters()
                        Locationsearchfilter[0] = new nlobjSearchFilter(
                            'custrecordintegrationlibposid',
                            null,
                            'equalTo',
                            ShopNum
                        )
                        ...
                        */
                        ...
                        checkGovernance()
                        var Channelsearchfilter = ChannelSearch.getFilters()
                        Channelsearchfilter[0] = new nlobjSearchFilter(
                            'custrecordintegrationlibposid',
                            null,
                            'equalTo',
                            ShopNum
                        ...
                        // searchfilter[1] = new nlobjSearchFilter('cseg', null, 'is', 86);
                        search.setFilters(searchfilter)
                        var resultSet = search.runSearch()
                        checkGovernance()
                        var searchresults = resultSet.getResults(0, 1)
                        ...
                        if (JTxnDetail) {
                            if (completed) {
                                ...
                                {
                                    if (!Cancelled) {
                                        var TxSearch = nlapiLoadSearch('transaction', 'customsearchtranfrompos_2')
                                        checkGovernance()
                                        var Txsearchfilter = TxSearch.getFilters()
                                        Txsearchfilter[1] = new nlobjSearchFilter(
                                            'custbodypossalesid',
                                            null,
                                            'is',
                                            SalesNum
                                        ...
                                        if (TxSearchresults.length == 0) {
                                            ...
                                            rec.setFieldValue('subsidiary', 14)
                                                ...
                                                for (var j = 0; j < JTxnDetail.length; j++) {
                                                    ...
                                                    if (lineCancelled == false) {
                                                        if (lineQTY > 0) {
                                                            ...
                                                            var ItemSearch = nlapiLoadSearch(
                                                                'item',
                                                                'customsearchpositem'
                                                            )
                                                            checkGovernance()
                                                            var Itemsearchfilter = ItemSearch.getFilters()
                                                            Itemsearchfilter[0] = new nlobjSearchFilter(
                                                                'custitem_barcode',
                                                                null,
                                                                'is',
                                                                lineProductCode
                                                            ...
                                                            ItemSearchColumn[0] = new nlobjSearchColumn(
                                                                'islotitem',
                                                                null
                                                            )
                                                            ...
                                                            if (ItemSearchresults.length > 0) {
                                                                ...
                                                                var islotitem = ItemSearchresults[0].getValue(
                                                                    columns[0]
                                                                )
                                                                var itemNum = ItemSearchresults[0].getValue(columns[4])
                                                                var displayname = ItemSearchresults[0].getValue(
                                                                    columns[1]
                                                                )
                                                                ...
                                                                rec.setCurrentLineItemValue(
                                                                    'item',
                                                                    'description',
                                                                    'item is not matching' + lineProductCode
                                                                )
                                                            }
                                                            ...
                                                            if (islotitem == 'T') {
                                                                ...
                                                                filters[2] = new nlobjSearchFilter(
                                                                    'location',
                                                                    null,
                                                                    'anyOf',
                                                                    INVlocation
                                                                ...
                                                                var LotNumSearch = nlapiCreateSearch(
                                                                    'inventorynumber',
                                                                    filters,
                                                                    columns
                                                                )
                                                                ...
                                                                checkGovernance()
                                                                var LotNumSearchresults =
                                                                    LotNumSearchresultSet.getResults(0, 1)
                                                                // nlapiLogExecution('debug', 'LotNumSearchresults.length', LotNumSearchresults.length);
                                                                var LotNum = ''
                                                                for (
                                                                    var m = 0;
                                                                    LotNumSearchresults.length > 0 &&
                                                                    LotNumSearchresults &&
                                                                    m == 0;
                                                                    m++
                                                                ) {
                                                                ...
                                                                if (LotNum != '') {
                                                                    ...
                                                                    var subrec = rec.createCurrentLineItemSubrecord(
                                                                        'item',
                                                                        'inventorydetail'
                                                                    )
                                                                    subrec.selectNewLineItem('inventoryassignment')
                                                                    subrec.setCurrentLineItemValue(
                                                                        'inventoryassignment',
                                                                    ...
                                                                    rec.setCurrentLineItemValue(
                                                                        'item',
                                                                        'description',
                                                                        'item has no stock. ' + lineProductCode
                                                                    )
                                                                    rec.setCurrentLineItemValue(
                                                                        'item',
                                                                        'custcolposremarks',
                                                            ...
                                                            recCM.setFieldValue('subsidiary', 14)
                                                            ...
                                                            if (islotitem == 'T') {
                                                                ...
                                                                if (LotNum != '') {
                                                                    ...
                                                                    var subrec = recCM.createCurrentLineItemSubrecord(
                                                                        'item',
                                                                        'inventorydetail'
                                                                    )
                                                                    subrec.selectNewLineItem('inventoryassignment')
                                                            ...
                                                            try {
                                                                ...
                                                                nlapiLogExecution(
                                                                    'error',
                                                                    'error Message: ' + err,
                                                                    JSON.stringify(SalesList[i])
                                                                )
                                                                var recError =
                                                                    nlapiCreateRecord('customrecordpossyncerror')
                                                                recError.setFieldValue(
                                                                    'custrecordsyncerror_posid',
                                                                    SalesNum
                                                                )
                                                                recError.setFieldValue(
                                                                    'custrecordsyncerror_json',
                                                                    JSON.stringify(SalesList[i])
                                                                )
                                                            ...
                                                            if (JTxnPayment) {
                                                                ...
                                                                for (var k = 0; k < JTxnPayment.length; k++) {
                                                                    ...
                                                                    if (PLineCA != true) {
                                                                        ...
                                                                        var NSpaymentMeth = 71
                                                                        if (PLineMethod == 0) {
                                                                            //Cash
                                                                            NSpaymentMeth = 71
                                                                        } else if (PLineMethod == 1) {
                                                                            //octopus
                                                                            NSpaymentMeth = 63
                                                                        } else if (PLineMethod == 2) {
                                                                            //Visa
                                                                            NSpaymentMeth = 56
                                                                        } else if (PLineMethod == 3) {
                                                                            //Master
                                                                            NSpaymentMeth = 56
                                                                        } else if (PLineMethod == 4) {
                                                                            //AE
                                                                            NSpaymentMeth = 55
                                                                        } else if (PLineMethod == 5) {
                                                                            //Union Pay
                                                                            NSpaymentMeth = 59
                                                                        } else if (PLineMethod == 6) {
                                                                            // Wechat Pay
                                                                            NSpaymentMeth = 62
                                                                        } else if (PLineMethod == 7) {
                                                                            // Alipay
                                                                            NSpaymentMeth = 61
                                                                        }
                                                                        ...
                                                                        // nlapiLogExecution('debug', 'CM_index',CM_index);
                                                                        var recPay = nlapiTransformRecord(
                                                                            'invoice',
                                                                            TXid,
                                                                            'customerpayment'
                                                                        )
                                                                        ...
                                                                        if (CM_index > 0) {
                                                                            var InvCrLineCount =
                                                                                recPay.getLineItemCount('credit')
                                                                            ...
                                                                            for (var o = 1; o <= InvCrLineCount; o++) {
                                                                                for (var n = 0; n < CM_index; n++) {
                                                                                    ...
                                                                                    var applied =
                                                                                        recPay.getLineItemValue(
                                                                                            'credit',
                                                                                            'apply',
                                                                                            o
                                                                                        )
                                                                                    var doc = recPay.getLineItemValue(
                                                                                        'credit',
                                                                                        'doc',
                                                                                        o
                                                                                    )
    ...
    nlapiSubmitField('customrecordintegrationlib', 99, 'custrecordintegrationlibdate', now)
    nlapiSubmitField('customrecordintegrationlib', 100, 'custrecordintegrationlibdate', DateEnd)
    checkGovernance()
...
function CheckError() {
    // Check Error URL
    // var CheckErrorURL = "http://ieh.softether.net:8888/erunapi_artisan/api.asmx/System_LastError"
    var CheckErrorURL = 'https://ithpos.app/erunapi_artisan/api.asmx/System_LastError'
...
function Login() {
    // Login URL
    // var LoginURL = "http://ieh.softether.net:8888/erunapi_artisan/api.asmx/System_Login?sUsername=998&sPassword=998"
    var LoginURL = baseUrl + '/System_LoginUser?sUsername=999&sPassword=999'
    // var LoginURL = "http://103.225.9.37:8888/erunapi_artisan/api.asmx/System_LoginUser?sUsername=999&sPassword=999"
...
function Logout() {
    // Logout URL
    // var LogoutURL = "http://ieh.softether.net:8888/erunapi_artisan/api.asmx/System_LogoutUser?sUsername=998"
    var LogoutURL = baseUrl + '/System_LogoutUser?sUsername=999'
    // var LogoutURL = "http://103.225.9.37:8888/erunapi_artisan/api.asmx/System_LogoutUser?sUsername=999"
...
function getSessionId(loginRes) {
    const cookies = loginRes.toString().split('; ')
    for (let i = 0; i < cookies.length; i++) {
        const cookie = cookies[i].split('=')
        if (cookie[0] === 'ASP.NET_SessionId') {
            log.debug('sessionId', cookie[1])
            return cookie[1]
        }
    }
