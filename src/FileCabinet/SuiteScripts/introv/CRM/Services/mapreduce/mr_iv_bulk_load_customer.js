 ...
 *
 * 20231227 Introv John add format & moment
 */
define(['N', 'N/format', '../../DAO/CustomerDAO', '../../Constants/ReportConstants', "../../DAO/EarnedRewardsDAO", 'N/search', '../../Constants/Constants', '../../../lib/Time/moment'], function (N_1, format, CustomerDAO, const_1, EarnedRewardsDAO, search, const_2, moment) {

    function getInputData() {
        let searchFilters = JSON.parse(N_1.runtime.getCurrentScript().getParameter({
            name: 'custscript_iv_search_filters_mr',
        }))
        let savingTimeStamp = N_1.runtime.getCurrentScript().getParameter({
            name: "custscript_iv_saving_time_stamp_mr"
        })

        bulkLoading(JSON.parse(searchFilters.nsFilters), JSON.parse(searchFilters.cnFilters), searchFilters.exportFromPageIndex, searchFilters.exportToPageIndex).then(function ([nsCustomersArrPromise, cnCustomersArrPromise]) {
    ...
    function nsSearch(nsFilters, exportFromPageIndex, exportToPageIndex) {
        ...
        const customerSearchColPointBalanceStored = N_1.search.createColumn({ name: "custentity_iv_cl_outstanding_pts_storage" })
        ...
        const customerIDSearch = N_1.search.create({
            ...
            columns: [
                ...
                customerSearchColPointBalanceStored
            ],
        })
        ...
        log.debug("Export Page", exportFromPageIndex + " | " + exportToPageIndex)
        for (let i = Number(exportFromPageIndex); i < Number(exportToPageIndex) + 1; i++) {
            ...
            for (let result of customerSearchData) {
                customerList.push({
                    ...
                    outstandingPoint: result.getValue(customerSearchColPointBalanceStored) || 0
            ...
            log.debug("Finished Customer Page Counter: ", i + 1 + " | Time: " + new Date())
            // break;
        }
    ...
    function cnFilterCustomerFullArr({ birthdayMonth, salutation, lastname, firstname, phone, customerid, age, referRef, dummyDate1, dummyDate2, dummyDate1To, dummyDate2To, dummyText1, dummyText2, marketing, tc, dLang, region, membership, email, district, regFromDate, regToDate, effectiveDate, effectiveDateTo, effectiveToDate, effectiveToTill, intrestedItems, pointBalanceFrom, pointBalanceTo }, cnCustomerFullArr) {
        ...
        var customerSearchObj = search.create({
            ...
            columns:
                [
                    ...
                    "custentity_iv_cl_outstanding_pts_storage"
                ]
        })
        let cnNSCustomerCount = customerSearchObj.runPaged().count
        var customerSearchResult = [];
        for (let i = 0; i < cnNSCustomerCount / 1000; i++) {
            let tempcustomerSearchResult = customerSearchObj.run().getRange({ start: i * 1000, end: i * 1000 + 999 })
            customerSearchResult = customerSearchResult.concat(tempcustomerSearchResult)
        }
        // const earnedRewardsDAO = new EarnedRewardsDAO()
        // let cnNSCustomerIDs = []
        ...
        for (let cnCustomer of cnCustomerFilterArr) {
            let pointBalance = 0
            for (let nsCNCustomer of customerSearchResult) {
                if (nsCNCustomer.getValue("entityid") == cnCustomer.CUSTOMER_ID) {
                    // log.debug("cnPointsMatched", pointDetails.customerID + " | " + pointDetails.pointBalance)
                    pointBalance = nsCNCustomer.getValue("custentity_iv_cl_outstanding_pts_storage")
                }
            }
    ...
    function bulkLoading(nsFilters, cnFilters, exportFromPageIndex, exportToPageIndex) {
        ...
        // let nsCustomerDataPromise = []
        //Do Ns first to prevent CN API pending for more than 1 hr
        // let nsCustomerDataPromise = new Promise(function (resolve, reject) {
        //     let nsCustomerList = nsSearch(nsFilters, exportFromPageIndex, exportToPageIndex)
        ...
        let cnCustomerDataPromise = new Promise(function (resolve, reject) {
            ...
            N_1.https.get.promise({
                ...
                N_1.https.get.promise({
                    ...
                    log.debug('error', error);
                    log.debug('error stack', error.stack);
                    log.error('Error occurred while retrieving customer data from CNDB(RETRY)', error);
                });
            });
        })
