		...
		if(memberList)
		{
			for(var i = 0; i < memberList.length; i++)
			{
				...
				else
				{
					...
					var filters = new Array();
					filters[0] = new nlobjSearchFilter( 'custrecordcustomersubcatecate', null, 'anyOf', 2);
					filters[1] = new nlobjSearchFilter( 'custrecordcustomersubcatposid', null, 'is', SGR);
					// filters[3] = new nlobjSearchFilter( 'quantityonhand', null, 'GREATERTHANOREQUALTO', lineQTY);
					// filters[2] = new nlobjSearchFilter( 'location', null, 'anyOf', Locationid);
					var columns = new Array();
					columns[0] = new nlobjSearchColumn('internalid');
...
{	
	...
	for (var xx = 7; xx > 0; xx--)
	{
		...
		// lastSyncDateTime = '2019/10/12 10:00:00';
		// lastSyncDateTimeDateEnd = '2019/10/12 23:00:00';
		// vBeginTime:	
		// 2020/01/17 13:40:00
		// vEndTime:	
		// 2020/01/17 14:00:00
		...
		var starttime = GetDateKeyDateStart(lastSyncDateTime);
		// nlapiLogExecution("debug","now",now);
		var requestBody = 'vBeginTime=' + starttime + '&vEndTime=' + lastSyncDateTimeDateEnd + '&bLU=false';
		...
		nlapiLogExecution("debug","requestBodyJSON",JSON.stringify(requestBody));
		...
		if(SalesList && SalesList != null && SalesList != 'null' && SalesList != '' )
		{
			...
			for(var i = 0; i < SalesList.length; i++)
			{
				...
				if(ShopNum != 99 && ShopNum != 0)
				{
					...
					if (SalesNum != 'R991908270220204312z')
					{	
						var ShopNum = SalesList[i]['SN'];
						...
						*/
						...
						if(DWD == 1)
						{
							...
							if(plus7Date)
							{
								...
								nlapiLogExecution('emergency', 'plus7Date: ' + plus7Date, 'TxnDate: ' + TxnDate);
							}
						}
